...
function buildCharts(sample) {
  d3.json("https://static.bc-edx.com/data/dl-1-2/m14/lms/starter/samples.json").then((data) => {
    ...
    let bar_markers = otu_ids.map(x => `Otu: ${x}`)

    //function makePlot(filtered_data) {
      // Create Trace
      let trace2 = {
        y: bar_markers.slice(0,10).reverse(),
    ...
    // Don't forget to slice and reverse the input data appropriately
      ...
      let layout2 = {
        title: {
          text: `Top Ten Bacteria Cultures`
        },
        
        xaxis: {
          title: {
            text: 'Bacteria Number'
          }
        },
      }
...
//This creates an option dropdown menu. 
